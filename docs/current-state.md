...
- **Immediate gates:** [#34](https://github.com/moldovancsaba/impact/issues/34) — maintainer publish + npm smoke; [#58](https://github.com/moldovancsaba/impact/issues/58) — **dashboard activation** (hosted ingest, live **`/api/stats/*`**, **`VITE_STATS_API_BASE`**, seed/smoke) — **parallel** where capacity ([mlp-status-cto.md § Leadership view](mlp-status-cto.md#cto-acceptance-leadership-dashboard)).
- **Top operational tasks:** **deploy ingest + wire web** · **publish npm** · [web-deploy-smoke.md](web-deploy-smoke.md) (§ **Live stats** when applicable). Repo **dashboard engine** is **in**; **product-live** stats are **not** until hosted proof.
...
- **MLP public web / stats track:** **[#50](https://github.com/moldovancsaba/impact/issues/50)–[#57](https://github.com/moldovancsaba/impact/issues/57)** (H1–H8) — multi-page shell shipped; **`/data.html`** supports **live** aggregates when built with **`VITE_STATS_API_BASE`** + reachable ingest  
